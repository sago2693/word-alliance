...
def save_model(model, optimizer, args, config, filepath,epoch, batch_size, weighted_avg,  dev_sentiment_accuracy, dev_paraphrase_accuracy, dev_sts_corr,loss):
    ...
    model_number = 1
    print(batch_size)
    while os.path.exists(os.path.join(filepath, f"{args.option}-epoch-number {epoch}-from-{args.epochs}-{args.lr}-model_{model_number}_batch_size_{batch_size}.pt")):
        model_number += 1

    model_path = os.path.join(filepath, f"{args.option}-epoch-number {epoch}-from-{args.epochs}-{args.lr}-model_{model_number}_batch_size_{batch_size}.pt")
    ...
    with open(txt_filename, 'w') as txt_file:
        ...
        txt_file.write(f"dev_paraphrase_accuracy: {dev_sts_corr}\n")
...
if __name__ == "__main__":
    ...
    test_model(args, ".\models\pretrain-epoch-number 0-from-10-0.001-model_1_batch_size_64-w-.pt")